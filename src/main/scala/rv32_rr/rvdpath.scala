...
// Dec 3, 2017
// 
// Data path, test mram and register file access
//=======================================================================
package rvcore_rr
...
class rvdpath extends Module {
    ...
    val alu_out = MuxLookup(
        ...
        Array(
            ...
            rvalu.ADD   -> (rs1_data + rs2_data).toUInt,
            rvalu.SUB   -> (rs1_data - rs2_data).toUInt,
            ...
            rvalu.SLL   -> (rs1_data << rs2_data(rvinst.shamtsz-1,0)).toUInt,
            rvalu.SLT   -> (rs1_data.toSInt < rs2_data.toSInt).toUInt,
            rvalu.SLTU  -> (rs1_data < rs2_data).toUInt,
            rvalu.XOR   -> (rs1_data ^ rs2_data).toUInt,
            rvalu.SRL   -> (rs1_data >> rs2_data(rvinst.shamtsz-1,0)).toUInt,
            rvalu.SRA   -> (rs1_data.toSInt >> rs2_data(rvinst.shamtsz-1,0)).toUInt,
            rvalu.OR    -> (rs1_data | rs2_data).toUInt,
            rvalu.AND   -> (rs1_data & rs2_data).toUInt
        )
